...
DISCORD_API_TIMEOUT = 15  # seconds
DISCORD_API_BASE_URL = "https://discord.com/api/v10"
DEFAULT_DISCORD_IMAGE_CHANNEL_ID = "1498585260290609202"
...
def _sync_development_database_from_production() -> None:
    """Atomically copy live SQLite contents into the development database."""
    production_path = (BASE_DIR / DATABASE_CHOICES["production"]["filename"]).resolve()
    development_path = (BASE_DIR / DATABASE_CHOICES["development"]["filename"]).resolve()
    if production_path == development_path:
        raise RuntimeError("Production and development database paths must differ.")
    with DATABASE_SWITCH_LOCK:
        source = sqlite3.connect(production_path)
        destination = sqlite3.connect(development_path)
        try:
            source.execute("PRAGMA query_only = ON")
            source.backup(destination)
            integrity = destination.execute("PRAGMA integrity_check").fetchone()
            if not integrity or integrity[0] != "ok":
                raise RuntimeError("Development database failed its integrity check after synchronization.")
        finally:
            destination.close()
            source.close()
...
def _discord_image_config() -> Optional[dict[str, str]]:
    bot_token = (os.getenv("DISCORD_BOT_TOKEN") or "").strip()
    channel_id = (os.getenv("DISCORD_IMAGE_CHANNEL_ID") or DEFAULT_DISCORD_IMAGE_CHANNEL_ID).strip()
...
def _send_discord_channel_image(
    config: dict[str, str],
    *,
    filename: str,
    file_bytes: bytes,
    ...
    url = f"{DISCORD_API_BASE_URL}/channels/{config['channel_id']}/messages"
    payload = {}
    if caption:
        payload["content"] = caption[:2000]
    files = {
    ...
    try:
        ...
        raise DiscordImageSendError(f"Unable to reach Discord ({exc})") from exc
    if response.status_code >= 400:
        ...
        try:
            ...
            message = None
        if not message:
            message = f"Discord error ({response.status_code})"
        raise DiscordImageSendError(message)
...
def switch_database():
    ...
    synced_from_production = current_choice == "production" and requested_choice == "development"
    if synced_from_production:
        try:
            _sync_development_database_from_production()
        except Exception:
            app.logger.exception("Unable to synchronize production data into the development database")
            return _redirect_with_query_params(
                next_url,
                db_switch_error="sync_failed",
            )
    _set_active_database(requested_choice)
    return _redirect_with_query_params(
        next_url,
        db_switched=requested_choice,
        db_synced="production" if synced_from_production else None,
...
@app.route("/api/discord/send-image", methods=["POST"])
@app.route("/api/whatsapp/send-image", methods=["POST"])
def send_discord_image():
    config = _discord_image_config()
    if not config:
        return jsonify({
            "ok": False,
            "error": "Discord image sending is not configured. Set DISCORD_BOT_TOKEN.",
        }), 503
    ...
    if len(file_bytes) > DISCORD_IMAGE_MAX_BYTES:
        limit_mb = DISCORD_IMAGE_MAX_BYTES // (1024 * 1024)
    ...
    if not mime_type.startswith("image/"):
        return jsonify({"ok": False, "error": "Only image files can be sent to Discord."}), 400

    caption = (request.form.get("caption") or "").strip()
    filename = upload.filename or f"schedule-{uuid.uuid4().hex}.png"

    try:
        _send_discord_channel_image(
            config,
            filename=filename,
    ...
    except DiscordImageSendError as exc:
        app.logger.warning("Discord image send error: %s", exc)
        return jsonify({"ok": False, "error": str(exc)}), 502

    return jsonify({"ok": True, "message": "Image sent to Discord."})
